...
if 'all_combos' not in st.session_state:
    st.session_state.all_combos = []
if 'fit_combo_idx' not in st.session_state:
    st.session_state.fit_combo_idx = None
if 'fit_elem_idx' not in st.session_state:
    st.session_state.fit_elem_idx = None
...
if st.button("Рассчитать"):
    ...
    st.dataframe(pd.DataFrame({'№': range(1, len(numbers)+1), 'Значение': numbers}), use_container_width=True)

    excel_tables = []
    all_combos = []  # Список для подгонки
    ...
    if excel_tables:
        excel_data = to_excel_onelayer(excel_tables)
        st.download_button(
...
def fit_dialog():
    ...
    st.session_state.fit_combo_idx = combo_idx
    ...
    st.session_state.fit_elem_idx = elem_idx
...
if st.button("Подогнать"):
    # Проверка на наличие расчётов
    if not st.session_state.get('all_combos'):
